...
namespace Oci.OsmanagementhubService
{
    ...
    public class LifecycleEnvironmentClient : RegionalClientBase
    {
        ...
        public async Task<RebootLifecycleStageResponse> RebootLifecycleStage(RebootLifecycleStageRequest request, RetryConfiguration retryConfiguration = null, CancellationToken cancellationToken = default, HttpCompletionOption completionOption = HttpCompletionOption.ResponseContentRead)
        {
            ...
            Uri uri = new Uri(this.restClient.GetEndpoint(), System.IO.Path.Combine(basePathWithoutHost, "/lifecycleStages/{lifecycleStageId}/actions/reboot".Trim('/')));
            HttpMethod method = new HttpMethod("POST");
            HttpRequestMessage requestMessage = Converter.ToHttpRequestMessage(uri, method, request);
            requestMessage.Headers.Add("Accept", "application/json");
            GenericRetrier retryingClient = Retrier.GetPreferredRetrier(retryConfiguration, this.retryConfiguration);
            ...
            try
            {
                ...
                {
                    ServiceName = "LifecycleEnvironment",
                    OperationName = "RebootLifecycleStage",
                    RequestEndpoint = $"{method.Method} {requestMessage.RequestUri}",
                ...
                this.restClient.CheckHttpResponseMessage(requestMessage, responseMessage, apiDetails);
                logger.Debug($"Total Latency for this API call is: {stopWatch.ElapsedMilliseconds} ms");
                return Converter.FromHttpResponseMessage<RebootLifecycleStageResponse>(responseMessage);
            }
            catch (OciException e)
            {
                logger.Error(e);
            ...
            {
                logger.Error($"RebootLifecycleStage failed with error: {e.Message}");
                throw;
            }
        }
