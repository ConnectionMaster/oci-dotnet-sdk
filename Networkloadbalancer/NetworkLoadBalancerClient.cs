...
namespace Oci.NetworkloadbalancerService
{
    ...
    public class NetworkLoadBalancerClient : RegionalClientBase
    {
        ...
        {
            if (!DeveloperToolConfiguration.IsServiceEnabled("networkloadbalancer"))
            {
        ...
        public async Task<GetBackendOperationalStatusResponse> GetBackendOperationalStatus(GetBackendOperationalStatusRequest request, RetryConfiguration retryConfiguration = null, CancellationToken cancellationToken = default, HttpCompletionOption completionOption = HttpCompletionOption.ResponseContentRead)
        {
            ...
            Uri uri = new Uri(this.restClient.GetEndpoint(), System.IO.Path.Combine(basePathWithoutHost, "/networkLoadBalancers/{networkLoadBalancerId}/backendSets/{backendSetName}/backends/{backendName}/operationalStatus".Trim('/')));
            HttpMethod method = new HttpMethod("GET");
            HttpRequestMessage requestMessage = Converter.ToHttpRequestMessage(uri, method, request);
            requestMessage.Headers.Add("Accept", "application/json");
            GenericRetrier retryingClient = Retrier.GetPreferredRetrier(retryConfiguration, this.retryConfiguration);
            ...
            try
            {
                ...
                {
                    ServiceName = "NetworkLoadBalancer",
                    OperationName = "GetBackendOperationalStatus",
                    RequestEndpoint = $"{method.Method} {requestMessage.RequestUri}",
                ...
                this.restClient.CheckHttpResponseMessage(requestMessage, responseMessage, apiDetails);
                logger.Debug($"Total Latency for this API call is: {stopWatch.ElapsedMilliseconds} ms");
                return Converter.FromHttpResponseMessage<GetBackendOperationalStatusResponse>(responseMessage);
            }
            catch (OciException e)
            {
                logger.Error(e);
            ...
            {
                logger.Error($"GetBackendOperationalStatus failed with error: {e.Message}");
                throw;
            }
        }
