...
### Breaking Changes
 - The `UsageRecordId` property was removed from the `CreateSubscriptionUsageRecordDetails` model in the Service Enablement Lifecycle Framework service
...
### Breaking Changes
 - The clients `CancelEnrichmentJobClient`, `GenerateEnrichmentJobClient`, `GetEnrichmentJobClient`, and `ListEnrichmentJobsClient` were removed from the Generative AI Data service
 - The client `GenerateSqlFromNlJobClient` was renamed to `GenerateSqlClient` in the Generative AI Data service
 - The paginator `ListEnrichmentJobsPaginators` was renamed to `EnrichmentJobPaginators` in the Generative AI Data service
 - The waiter `GetEnrichmentJobWaiters` was renamed to `EnrichmentJobWaiters` in the Generative AI Data service
...
### Breaking Changes
 - The field CompartmentId was added as a required field to the `ListApiMetadataByEntityTypesRequest`, `ListApiMetadataRequest`, `ListPrivilegedApiControlsRequest`, `ListPrivilegedApiRequestsRequest`, and `ListWorkRequestsRequest` request models in the Oracle API Access Control service
 - The field CompartmentId was added as a required field to the `CreatePrivilegedApiRequestDetails` model in the Oracle API Access Control service
...
### Breaking Changes
 - Type of field `LifecycleState` was changed to `HostedApplicationBase.LifecycleStateEnum` in the model `HostedApplicationSummary` in the Generative AI service
...
### Breaking Changes 
 - The fields `Id`, `ResolverId`, `FreeformTags`, and `DefinedTags` were added as required fields to the models `ResolverEndpoint` and `ResolverEndpointSummary` in the DNS service 
 - The field `SecurityAttributes` was added as a required field to the model `ResolverVnicEndpointSummary` in the DNS service
...
### Breaking Changes
 - Invalid service endpoints now throw `ArgumentException` instead of `UriFormatException`
...
### Breaking Changes  
 - The fields AvailabilityDomain and SubnetId were made mandatory in the model PlacementConfiguration in the Kubernetes Engine service  
 ...
 - The enum values Idcs and IdcsCustomService were removed from the model AuthConfiguration.TypeEnum in the Data Science service
...
### Breaking Changes
 - The fields `ClusterAdminPassword` and `SecretId` were removed from the model `CreateBdsCertificateConfigurationDetails` in the Big Data service
...
### Breaking Changes
 - Added a required parameter `AuthenticationType` for `DatabaseToolsConnectionOracleDatabase` operations in the Database Tools service
 - Added a required parameter `ConnectivityType` for `DigitalTwinInstance` operations in the OCI Internet of Things (OCI IoT) service
...
### Breaking Changes
 ...
 - The model `DeploymentUpgradeLifecycleState` was removed in the GoldenGate service
 - Enum value `GoldengateConnectionAssigmnentDelete` was removed from model `OperationType` in the GoldenGate service
 - The field `TimeUpgradeRequired` was removed in the model Deployment in the GoldenGate service 
 - Type of field `LifecycleState` was changed to `LifecycleStateEnum` in the models `DeploymentBackupSummary`, `DeploymentBackup`, `DeploymentSummary`, `DeploymentUpgradeSummary`, `DeploymentUpgrade`, `Deployment`, `ListDeploymentBackupsRequest`, `ListDeploymentPeersRequest`, `ListDeploymentUpgradesRequest`, and `ListDeploymentsRequest` in the GoldenGate service
 - Type of field `BootstrapServers` was changed from `KafkaBootstrapServer` to `CreateKafkaBootstrapServe` in the model `CreateKafkaConnectionDetails` in the GoldenGate service
...
### Breaking Changes
 - Added a required parameter `ResolutionMode` for Zone operations in the DNS service
...
### Breaking Changes
 ...
 - Required fields `RuntimeEndpoint` and `RuntimeIdentity` were added to `DatabaseToolsConnection` operations in the Database service
 - Required field `IsAutomatic` was added to `DrPlanExecution` operations in the Disaster Recovery service
...
### Breaking Changes
 - The return type of property `CvssScoreGreaterThan` and `CvssScoreLessThan` was changed to `float` in the request `SummarizeLibraryUsageRequest` in the Java Management Service
