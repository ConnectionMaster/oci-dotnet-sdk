...
using Oci.ClusterhealthService.Models;

namespace Oci.ClusterhealthService
{
    /// <summary>
    /// Collection of helper methods that can be used to provide an enumerator interface
    /// to any list operations of Diagnosis where multiple pages of data may be fetched.
    ...
    /// </summary>
    public class DiagnosisPaginators
    {
        private readonly DiagnosisClient client;

        public DiagnosisPaginators(DiagnosisClient client)
        {
            this.client = client;
        }

        /// <summary>
        /// Creates a new enumerable which will iterate over the responses received from the ListDiagnosisStores operation. This enumerable
        ...
        public IEnumerable<ListDiagnosisStoresResponse> ListDiagnosisStoresResponseEnumerator(ListDiagnosisStoresRequest request, Common.Retry.RetryConfiguration retryConfiguration = null, CancellationToken cancellationToken = default)
        {
            return new Common.Utils.ResponseEnumerable<ListDiagnosisStoresRequest, ListDiagnosisStoresResponse>(
                ...
                request => client.ListDiagnosisStores(request, retryConfiguration, cancellationToken)
            );
        }

        /// <summary>
        /// Creates a new enumerable which will iterate over the DiagnosisStoreSummary objects
        /// contained in responses from the ListDiagnosisStores operation. This enumerable will fetch more data from the server as needed.
        /// </summary>
        /// <param name="request">The request object containing the details to send</param>
        /// <param name="retryConfiguration">The configuration for retrying, may be null</param>
        /// <param name="cancellationToken">The cancellation token object</param>
        /// <returns>The enumerator, which supports a simple iteration over a collection of a specified type</returns>
        public IEnumerable<DiagnosisStoreSummary> ListDiagnosisStoresRecordEnumerator(ListDiagnosisStoresRequest request, Common.Retry.RetryConfiguration retryConfiguration = null, CancellationToken cancellationToken = default)
        {
            return new Common.Utils.ResponseRecordEnumerable<ListDiagnosisStoresRequest, ListDiagnosisStoresResponse, DiagnosisStoreSummary>(
                response => response.OpcNextPage,
                input =>
                {
                    if (!string.IsNullOrEmpty(input))
                ...
                request => client.ListDiagnosisStores(request, retryConfiguration, cancellationToken),
                response => response.DiagnosisStoreCollection.Items
            );
        }
